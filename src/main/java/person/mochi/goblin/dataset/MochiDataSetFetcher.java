...
public class MochiDataSetFetcher extends BaseDataFetcher {
	...
	public MochiDataSetFetcher() {
		try {
			...
			this.totalExamples = (int) (positiveResults.size() + (int) negativeResults.size());
			ratio = (double) positiveResults.size() / (double) negativeResults.size();
	...
	public void fetch(int numExamples) {
		...
		int positiveExamples = (int) (numExamples * ratio > 1? numExamples * ratio: 1);
		int negativeExamples = numExamples - positiveExamples;
		float[][] featureData = new float[numExamples][0];
		float[][] labelData = new float[numExamples][0];
		int actualExamples = 0;
		int positiveExamplesIndex = 0;
		int negativeExamplesIndex = 0;
		for (int i = 0; i < numExamples; i++, cursor++) {
			...
			} else if(positiveExamplesIndex < positiveExamples && positiveResults.hasNext()) {
...
//		MongoDBConfigWithAuth confNegatives = new MongoDBConfigWithAuth("127.0.0.1", 27017, "mochi",
//				"gotohellmyevilex", new String[] { "data", "articleId" }, null);
//		MongoDBPlayer playerArticles = new MongoDBPlayer(confArticles, "Yihan", "Articles");
//		MongoDBPlayer playerPositives = new MongoDBPlayer(confPositives, "Yihan", "Positives");
//		MongoDBPlayer playerNegatives = new MongoDBPlayer(confNegatives, "Yihan", "Negatives");
//		Duality dual = new Duality();
